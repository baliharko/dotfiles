...
Run `./tmux/install_plugins.sh` to clone `tpm` and install the plugins declared in `tmux/.config/tmux/.tmux.conf`.
The script is idempotent and can be re-run after updating plugin definitions.

## zsh dependencies

Run `./zsh/install.sh` to set up oh-my-zsh, clone `zsh-syntax-highlighting` and `zsh-autosuggestions`, and install Starship (uses Homebrew when available).
